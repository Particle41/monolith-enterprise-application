...
Every version lives in `image-pins.env` beside this file, pinned deliberately
rather than tracking latest. The build copies it in and each install step
sources it, so there is one place to look and nothing kept in agreement by
hand. It ships inside the image as well, at `/usr/local/share/image-pins.env`,
so the environment can always be asked what it is made of.

The base image is the one exception: it is a literal digest in the Dockerfile,
because a `FROM` line is resolved before any file can be read. It is pinned to
a multi-architecture index digest, and the image is published for
`linux/amd64` and `linux/arm64`, so an Apple Silicon machine runs it natively.
