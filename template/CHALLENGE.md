...
- **No SQL statement logging left on, and no credentials in the tree.** The
  repository is scanned for secrets, including any we issued you. The scan is
  a literal one, over source and configuration files, and it looks for four
  things: the string `show_sql`; an `AKIA`-prefixed AWS access key ID; a
  quoted `password = "..."` assignment whose value is eight characters or
  more; and a quoted `secret_key = "..."` assignment. The last two match on
  shape, so a fixture or an example with a long quoted password in it trips
  them exactly as a real credential would.
